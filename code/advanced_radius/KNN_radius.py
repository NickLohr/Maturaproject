...
def parse_line(line):
    ...
    """
    if "FAIL" in line:
        return ("unkown",datetime.datetime(1970,1,1))
    if not "OK" in line:
        print(line)
        return ("wierd parsing",datetime.datetime(1970,1,1))
...
if __name__ == "__main__":
    ...
    for m in oo:
        for k in oo[m]:
            mini = min(oo[m][k])
            maxi = max(oo[m][k])
            op[m][k]=[mini.hour*3600+mini.minute*60+mini.second,maxi.hour*3600+maxi.minute*60+maxi.second]
            days.append(k)
    oq = collections.defaultdict(list)
