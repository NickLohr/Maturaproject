...
def multi_fit_to_lesson(liste):
    fit_to_lesson(liste[0],liste[1],liste[2],filter_function=filter_mac_all)
def multi_track(filenames, aps, time_table_dic, pooli):
    processes = []
    fil = len(filenames)
    for k in range(1,1+pooli):
        processes.append([list(filenames[int((k-1)/pooli*fil):int((k)/pooli*fil))],dict(aps),dict(time_table_dic)])
    print(len(processes))
    p = Pool(pooli)
    p.map(multi_fit_to_lesson,processes)
