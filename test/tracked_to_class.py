...
ddaattee = str(input("Date like: 2019-Jan-30 (30.11.2018-30.1.2019) "))    
...
def sl_to_num(num):
    dictt={"Jan":1,"Feb":2,"Nov":11,"Dec":12}
    return dictt[num]
...
def list_to_per(liste):
    ...
    if total==0:
        return 0
    if dic[max(dic, key=dic.get)]>dicte[max(dicte, key=dicte.get)]:
        #print out the most often used classes
        print (max(dic, key=dic.get)+ " to " + str(dic[max(dic, key=dic.get)]/total*100)+"%" )
        #print (max(dicte, key=dicte.get)+ " to " + str(dicte[max(dicte, key=dicte.get)]/total*100)+"%" )
    else:
        print (max(dicte, key=dicte.get)+ " to " + str(dicte[max(dicte, key=dicte.get)]/total*100)+"%" )
